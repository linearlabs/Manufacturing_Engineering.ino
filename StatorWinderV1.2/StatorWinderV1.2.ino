//Stator Winder, Arduino MEGA, V1.2
//Doug Gammill 3/8/22
//Linear Labs Inc.
...
String idn = "Stator Winder, Arduino MEGA, V1.2";
bool footSwitch_Val;
    ...
    if (upVal == 0) {
      ...
      if (valueX != rotationX) { // Use the DT pin to find out which way were turning.
        if (digitalRead(DT) != valueX) {  // Clockwise
          currentPositionX = currentPositionX + 4;
          LeftRight = true;
        } else { //Counterclockwise
          LeftRight = false;
          currentPositionX = currentPositionX - 4;
      ...
      rotationX = valueX;
      stepperX.moveTo(currentPositionX);
      stepperX.run();
    }
  }
}
...
void FootSwac() {
  ...
  if ((footSwitch_Val == 0) && (footSwitch_Index == 0) && (clockWise == true) && (firstPole == 0.5) && (poleNum == 1)) { // do not move y
    ...
    printPole();
    currentPositionX = currentPositionX + 300; // 300 steps = 180deg rotation (positive = CW)
  ...
  // only Foot switch activated
    ...
    // A1, or C1 turns CCW
      ...
      printPoleNot();
      currentPositionX = currentPositionX - 300; // 300 steps = 180deg rotation (negative = CCW)
      stepperX.runToNewPosition(currentPositionX);
      updateLCD();
    }
    ...
    // a2, or c2 turns CW
      ...
      firstPole = firstPole + 0.5;
      printPole();
      currentPositionX = currentPositionX + 300; // 300 steps = 180deg rotation  (positive = CCW)
      stepperX.runToNewPosition(currentPositionX);
      updateLCD();
    }
    ...
    //Finish step,  a8 move to B phase winding or c8 move back to begining
      ...
      poleNumNot = 9;
      if (startOK == "a") {
        index = 13; // moveY 13 poles to start phaseB
      }
      else if (startOK == "c") {
        index = 1; // moveY 1 pole to move back to phaseA
      }
      moveY();
      currentPositionX = currentPositionX + 300; // 300 steps = 180deg rotation  (positive = CW)
      stepperX.runToNewPosition(currentPositionX);

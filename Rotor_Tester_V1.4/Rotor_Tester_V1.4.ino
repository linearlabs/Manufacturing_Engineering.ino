//FIX ME: Add a proper header
//magnet testing cnc machine
...
//#define sensorA0 A0		// FIX ME: are these analog?
//#define sensorA1 A1
//#define sensorA2 A2

AccelStepper stepperX = AccelStepper(motorInterfaceType, stepPinX, dirPinX); // FIX ME: what are these?
...
int VRxA5;                                        //CH1 hall sensor
int VRx;
int VRyA4;                                        //CH2 hall sensor
int VRy;
...
int i = 1;                                        //sets magnet counter to start at 1
int j = 1;
unsigned long millisK;                            //timer for 500ms sampling
float magNum = 0;                                   //variable for number of magnets a rotor has
int xStep;
...
int dZ = 0;					// FIX ME: what is this for? Comment these vars
int dY = 0;
int flagX1 = 0;					// FIX ME: What are these flags for?
int flagX2 = 0;
char startCond;
String inputStr;				// FIX ME: explain these vars
char startChar;
...
void setup() {
  ...
  pinMode(SwX, INPUT);				// FIX ME: what are these?
  pinMode(SwY, INPUT);
  pinMode(SwZ, INPUT);
  stepperX.setCurrentPosition(0);		// FIX ME: what are these?
  ...
  sync();                                                       //whats for a : character to initiate
...
void Run() {		//FIX ME: put code in order where you don't call yet to be defined functions
  runY();
  runZ();
  runX();
}
...
// FIX ME: more comments from here on down
void homeY() {
  if (valSwY == LOW) {
    digitalWrite(en_Y, LOW);
    valY = digitalRead(en_Y);
...
// Notes:
// Not bad.
// I give you a C+. It would have been better if I could follow the code more clearly.
// Let's get you to A-level by the end of next week. I think you have a talent for
// coding - you just need a bit of refinement to get to that A.

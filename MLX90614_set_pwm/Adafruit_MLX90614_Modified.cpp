...
uint16_t Adafruit_MLX90614::readPWMCTRLReg(void) {   //1
...
void Adafruit_MLX90614::writePWMCTRLReg(uint16_t ereg) {
  write16(MLX90614_PWMCTRL, 0); // erase
  delay(10);
  write16(MLX90614_PWMCTRL, ereg);
  delay(10);
}
/**
...
double Adafruit_MLX90614::readPWMCTRL(void) {
  uint16_t ereg = read16(MLX90614_PWMCTRL);
  if (ereg == 0)
    return NAN;
  //return ((double)ereg) / 65535.0;
  return (ereg);
...
void Adafruit_MLX90614::writePWMCTRL(double PWM_MODE) {
  uint16_t ereg = (uint16_t)(PWM_MODE);

  writePWMCTRLReg(ereg);
}
/**

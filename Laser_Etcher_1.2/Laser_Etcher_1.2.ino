...
void entSnMenu() {
  ...
  while (snMenuIndex == 1) {
    ...
    else if ((entVal == LOW) && (RDoorVal == HIGH) && (LDoorVal == HIGH)) {
      if (entIndex == 1) {
        laserIndex = 1;
        sendCmd();
        deleteWriteConfig();
...
void printDirectory(File dir, int numTabs) {
  ...
  if (!entry) {                 //manual search allows infinite looping
    dir.rewindDirectory();
    sdSearchTries++;              //scaning auto search tries time out after 2
    printDirectory(root, 0);
  }
  ...
  if (entry) {
    ...
    if (jobNumFile.equalsIgnoreCase(sysFolderName)) {   //if statement skips system folder "SYSTEM~1"
      ...
      //StringSplitter *splitter = new StringSplitter(jobNumFile, '.', 2);  // new StringSplitter(string_to_split, delimiter, limit)
      //int itemCount = splitter->getItemCount();
      //String item = splitter->getItemAtIndex(0);   //item index is the part in the string to display
      //jobNum = String (item);
    }
    entry.close();
  }
